...
drawtext(const char *text, XftColor col[ColLast])
{
    ...
    XftDrawStringUtf8(d, &col[ColFG], dc.font.xfont, x, y,
		      (XftChar8 *) buf, len);
    XftDrawDestroy(d);
}
...
getcolor(const char *colstr)
{
	...
	(dpy, DefaultVisual(dpy, screen), DefaultColormap(dpy, screen),
	 colstr, &color))
...
propertynotify(const XEvent * e)
{
	...
	if (wmh->flags & XUrgencyHint) {
		...
		if (urgentswitch && wmh && !(wmh->flags & XUrgencyHint)) {
		    /*
		     * only switch, if tabbed was focused since last
		     * urgency hint if WMHints could not be received,
		     * default to no switch 
		     */
		    focus(c);
		} else {
		    /*
		     * if no switch should be performed, mark tab as
		     * urgent 
		     */
