...
struct Monitor {
	...
	int wx, wy, ww, wh;   /* window area  */
 	int win_count;		  /* move that many clients forward */
...
switch_windows_end()
{
	...
	*/
	if (selmon->nTabs > 1) {
		if (selmon->win_count != 0) { /* if user picked original client do nothing */
...
draw_list(int nwins, int first, Monitor *m)
{
	...
	if (first) {
		...
		m->tabwin = XCreateWindow(dpy, root, posX, posY, selmon->menu_max_width, selmon->menu_max_height, 2, DefaultDepth(dpy, screen),
	...
	drw_map(drw, m->tabwin, 0, 0, selmon->menu_max_width, selmon->menu_max_height);
}
