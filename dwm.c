/*
 * LICENSE file for copyright and license details. dynamic window manager 
 * is designed like any other X client as well. It is driven through
 * handling X events. In contrast to other X clients, a window manager
 * selects for SubstructureRedirectMask on the root window, to receive
 * events about window (dis-)appearance. Only one X connection at a time
 * is allowed to select for this event mask. The event handlers of dwm
 * are organized in an array which is accessed whenever a new event has
 * been fetched. This allows event dispatching in O(1) time. Each child
 * of the root window is called a client, except windows which have set
 * the override_redirect flag. Clients are organized in a linked client
 * list on each monitor, the focus history is remembered through a stack
 * list on each monitor. Each client contains a bit array to indicate the
 * tags of a client. Keys and tagging rules are organized as arrays and
 * defined in config.h. To understand everything else, start reading
 * main(). 
 */
#include <errno.h>
#include <locale.h>
...
void            draw_list(int nwins, int first, Monitor * m);
void            switch_windows_start(const Arg * arg);
static void     switch_windows_end();
...
static int      screen;
static int      sw,
                sh;		/* X display screen geometry width, height 
				 */
...
configurerequest(XEvent * e)
{
	...
	if (ev->value_mask & CWBorderWidth)
		...
		c->x = m->mx + (m->mw / 2 - WIDTH(c) / 2);	/* center
								 * in x
								 * direction 
								 */
	    if ((c->y + c->h) > m->my + m->mh && c->isfloating)
		c->y = m->my + (m->mh / 2 - HEIGHT(c) / 2);	/* center
								 * in y
								 * direction 
								 */
...
switch_windows()
{
    ...
    draw_list(selmon->nTabs, 0, selmon);
}
...
switch_windows_end()
{
     ...
     */
    if (selmon->nTabs > 1) {
	if (selmon->win_count != 0) {	/* if user picked original client
					 * do nothing */
	    Client         *buff = selmon->altsnext[selmon->win_count];
	    if (selmon->win_count > 1)
		for (int i = selmon->win_count; i > 0; i--)
		    selmon->altsnext[i] = selmon->altsnext[i - 1];
	    else		/* swap them if there are just 2 clients */
		selmon->altsnext[selmon->win_count] = selmon->altsnext[0];
...
void
draw_list(int nwins, int first, Monitor * m)
{
    /*
     ...
     */
    /*
     * int drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int
     * h, unsigned int lpad, const char *text, int invert); 
     */
    /*
	...
	selmon->menu_max_width = menu_max_width;
	selmon->menu_max_height = menu_max_height;
	...
	if (menu_position_x == 1)
	    posX += (selmon->mw / 2) - (menu_max_width / 2);
	if (menu_position_x == 2)
	    posX += selmon->mw - menu_max_width;

	if (menu_position_y == 0)
	    posY += selmon->mh - menu_max_height;
	if (menu_position_y == 1)
	    posY += (selmon->mh / 2) - (menu_max_height / 2);
	if (menu_position_y == 2)
	    posY += 0;

	h = selmon->menu_max_height;
	/*
	 * XCreateWindow(display, parent, x, y, width, height,
	 * border_width, depth, class, visual, valuemask, attributes);
	 * just reference 
	 */
	m->tabwin = XCreateWindow(dpy, root, posX, posY, selmon->menu_max_width, selmon->menu_max_height, 2, DefaultDepth(dpy, screen), CopyFromParent, DefaultVisual(dpy, screen), CWOverrideRedirect | CWBackPixmap | CWEventMask, &wa);	/* create 
																														 * tabwin 
																														 */
    ...
    h = selmon->menu_max_height / m->nTabs;

    int             y = 0;
    int             n = 0;
    for (int i = 0; i < m->nTabs; i++) {	/* draw all clients into
						 * tabwin */
	c = m->altsnext[i];
	if (!ISVISIBLE(c))
	    continue;
	/*
	 * if (HIDDEN(c)) continue; uncomment if you're using awesomebar
	 * patch 
	 */

	n++;
	drw_setscheme(drw, scheme[(c == m->sel) ? SchemeSel : SchemeNorm]);
	drw_text(drw, 0, y, selmon->menu_max_width, h, 0, c->name, 0);
	y += h;
    }

    drw_setscheme(drw, scheme[SchemeNorm]);
    drw_map(drw, m->tabwin, 0, 0, selmon->menu_max_width,
	    selmon->menu_max_height);
}

void
switch_windows_start(const Arg * arg)
{
	...
	if (m->nTabs > 0) {
		...
		if (XGrabKeyboard
		    (dpy, DefaultRootWindow(dpy), True, GrabModeAsync,
		...
		while (grabbed) {
			...
			if (event.type == KeyRelease) {	/* if super key is 
							 * released break
							 * cycle */
			    ...
			    if (event.xkey.keycode == 0x32)
				shift = ~shift;
			    if (event.xkey.keycode == mod_key_cycle) {	/* if 
									 * XK_s 
									 * is 
									 * pressed 
									 * move 
		 ...
		 */
		XUngrabKeyboard(dpy, CurrentTime);	/* stop taking all 
							 * input from
							 * keyboard */
		XWarpPointer(dpy, None, c->win, 0, 0, 0, 0, c->w / 2,
			     c->h / 2);
...
drawbar(Monitor * m)
{
     ...
     */
    if (m == selmon) {		/* status is only drawn on selected
				 * monitor */
	drw_setscheme(drw, scheme[SchemeNorm]);
	...
	drw_setscheme(drw,
		      scheme[m->
			     tagset[m->
				    seltags] & 1 << i ? SchemeSel :
			     SchemeNorm]);
	drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
	if (occ & 1 << i)
	    drw_rect(drw, x + boxs, boxs, boxw, boxw,
		     m == selmon && selmon->sel
		     && selmon->sel->tags & 1 << i, urg & 1 << i);
	x += w;
    }
    w = TEXTW(m->ltsymbol);
    drw_setscheme(drw, scheme[SchemeNorm]);
	...
	if (n > 0) {
		...
		drw_setscheme(drw,
			      scheme[m == selmon
				     && m->sel ==
				     c ? SchemeSel : SchemeNorm]);
		if (tw > lrpad / 2)
		    drw_text(drw, x, 0, tw, bh, lrpad / 2, c->name, 0);
		if (c->isfloating) {
		    drw_rect(drw, x + boxs, boxs, boxw, boxw, c->isfixed,
			     0);
		    if (c->isalwaysontop)
			drw_rect(drw, x + boxs, bh - boxw, boxw, boxw, 0,
				 0);
		}
		x += tw;
		w -= tw;
...
manage(Window w, XWindowAttributes * wa)
{
	...
	*term = NULL;
    ...
    c->x = c->mon->mx + (c->mon->mw - WIDTH(c)) / 2 - (rand() % WIDTH(c));
    c->y =
	...
	Atom            atom;
	if (XGetWindowProperty
	    (dpy, c->win, netatom[NetClientInfo], 0L, 2L, False,
	     XA_CARDINAL, &atom, &format, &n, &extra,
	     (unsigned char **) &data) == Success && n == 2) {
	    c->tags = *data;
	    for (m = mons; m; m = m->next) {
		if (m->num == *(data + 1)) {
		    c->mon = m;
...
togglefloating(const Arg * arg)
{
    if (!selmon->sel)
	return;
    if (selmon->sel->isfullscreen)	/* no support for fullscreen
					 * windows */
	return;
...
updatewmhints(Client * c)
{
	...
	if (c == selmon->sel && wmh->flags & XUrgencyHint) {
		...
		XSetWindowBorder(dpy, c->win,
				 scheme[SchemeUrg][ColBorder].pixel);
	}
...
main(int argc, char *argv[])
{
	...
	die("dwm: cannot open xcb connection");
    checkotherwm();
    setup();
#ifdef __OpenBSD__
