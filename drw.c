...
apply_fribidi(const char *str)
{
	...
	char           *output = ecalloc(len, sizeof(char *));
	len = fribidi_unicode_to_charset(charset, visual, len, output);
	return output;
    }
    return NULL;
}
...
drw_text(Drw * drw, int x, int y, unsigned int w, unsigned int h,
	 unsigned int lpad, const char *stext, int invert)
{
    int             i,
                    ty,
                    ellipsis_x = 0;
    ...
    char           *sstext = apply_fribidi(stext);
    char           *text = sstext;
	...
	while (*text) {
		...
		if (charexists) {
			...
			overflow = 1;
			/*
			 * called from drw_fontset_getwidth_clamp(): it
			 * wants the width AFTER the overflow 
			 */
			if (!render)
			    x += tmpw;
			else
	...
	if (!*text || overflow) {
	    ...
	    match =
		XftFontMatch(drw->dpy, drw->screen, fcpattern, &result);
