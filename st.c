...
typedef struct {
    int             tw,
                    th;		/* tty width and height */
    int             w,
                    h;		/* window width and height */
    int             ch;		/* char height */
    int             cw;		/* char width */
...
static int      xmakeglyphfontspecs(XftGlyphFontSpec *, const Glyph *, int,
				    int, int);
static void     xdrawglyphfontspecs(const XftGlyphFontSpec *, Glyph, int,
				    int, int);
...
mousesel(XEvent * e, int done)
{
    int             type,
                    seltype = SEL_REGULAR;
    uint            state =
	e->xbutton.state & ~(Button1Mask | forcemousemod);
...
mousereport(XEvent * e)
{
	...
	if (IS_SET(MODE_MOUSEMOTION) && buttons == 0)
	    return;
	/*
	 * Set btn to lowest-numbered pressed button, or 12 if no buttons
	 * are pressed. 
	 */
...
mouseaction(XEvent * e, uint release)
{
     ...
     */
    uint            state =
	e->xbutton.state & ~buttonmask(e->xbutton.button);

    for (ms = mshortcuts; ms < mshortcuts + LEN(mshortcuts); ms++) {
	if (ms->release == release && ms->button == e->xbutton.button && (match(ms->mod, state) ||	/* exact 
													 * or 
													 * forced 
													 */
									  match
...
selrequest(XEvent * e)
{
     ...
     */
    if (!XSendEvent
	(xsre->display, xsre->requestor, 1, 0, (XEvent *) & xev))
...
ximopen(Display * dpy)
{
    XIMCallback     imdestroy = {.client_data = NULL,.callback =
	    ximdestroy };
    XICCallback     icdestroy = {.client_data = NULL,.callback =
	    xicdestroy };
...
int
xmakeglyphfontspecs(XftGlyphFontSpec * specs, const Glyph * glyphs,
		    int len, int x, int y)
{
...
void
xdrawglyphfontspecs(const XftGlyphFontSpec * specs, Glyph base, int len,
		    int x, int y)
{
...
kpress(XEvent * ev)
{
    ...
    if (xw.ime.xic) {
	len =
	    XmbLookupString(xw.ime.xic, e, buf, sizeof buf, &ksym,
			    &status);
...
base64dec(const char *src)
{
    ...
    while (*src) {
	int             a =
	    base64_digits[(unsigned char) base64dec_getc(&src)];
	int             b =
	    base64_digits[(unsigned char) base64dec_getc(&src)];
	int             c =
	    base64_digits[(unsigned char) base64dec_getc(&src)];
...
tsetmode(int priv, int set, const int *args, int narg)
{
	...
	if (priv) {
		...
		break;
	    case 1049:		/* swap screen & set/restore cursor as
				 * xterm */
		if (!allowaltscreen)
		    break;
	    ...
	    case 1005:		/* UTF-8 mouse mode; will confuse
				 * applications not supporting UTF-8 and
				 * luit. */
...
csihandle(void)
{
	...
	break;
    case 'b':			/* REP -- if last char is printable print
				 * it <n> more times */
	DEFAULT(csiescseq.arg[0], 1);
	if (term.lastc)
